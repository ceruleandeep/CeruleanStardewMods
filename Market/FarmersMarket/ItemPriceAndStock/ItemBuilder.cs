...
namespace FarmersMarket.ItemPriceAndStock
{
    ...
    class ItemBuilder
    {
        ...
        public bool AddItemToStock(string itemName, double priceMultiplier = 1)
        {
            ...
            if (id < 0)
            {
                FarmersMarket.monitor.Log($"{_itemStock.ItemType} named \"{itemName}\" could not be added to the Shop {_itemStock.ShopName}", LogLevel.Trace);
                return false;
            }
        ...
        public bool AddItemToStock(int itemId, double priceMultiplier = 1)
        {
            ...
            if (FarmersMarket.VerboseLogging)
                FarmersMarket.monitor.Log($"Adding item ID {itemId} to {_itemStock.ShopName}", LogLevel.Debug);

            if (itemId < 0)
            {
                FarmersMarket.monitor.Log($"{_itemStock.ItemType} of ID {itemId} could not be added to the Shop {_itemStock.ShopName}", LogLevel.Trace);
                return false;
            }
            ...
            if (_itemStock.IsRecipe)
            {
                ...
                {
                    FarmersMarket.monitor.Log($"{item.Name} is not a valid recipe and won't be added.", LogLevel.Trace);
                    return false;
                }
            }
