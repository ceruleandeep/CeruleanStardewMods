...
namespace FarmersMarket.Utility
{
    /// <summary>
    ...
    public static class ItemsUtil
    {
        ...
        public static void UpdateObjectInfoSource()
        {
            ...
            ObjectInfoSource = new Dictionary<string, IDictionary<int, string>>
            {
                ...
                {
                    "Hat",
                    FarmersMarket.helper.Content.Load<Dictionary<int, string>>
            ...
            RecipesList = FarmersMarket.helper.Content.Load<Dictionary<string, string>>(@"Data/CraftingRecipes", ContentSource.GameContent).Keys.ToList();
            RecipesList.AddRange(FarmersMarket.helper.Content.Load<Dictionary<string, string>>(@"Data/CookingRecipes", ContentSource.GameContent).Keys.ToList());

            //add "recipe" to the end of every element
            RecipesList = RecipesList.Select(s => s + " Recipe").ToList();

            //load up tree and crop data
            _fruitTreeData = FarmersMarket.helper.Content.Load<Dictionary<int, string>>(@"Data/fruitTrees", ContentSource.GameContent);
            _cropData = FarmersMarket.helper.Content.Load<Dictionary<int, string>>(@"Data/Crops", ContentSource.GameContent);
        }
        ...
        {
            //int cropID = FarmersMarket.JsonAssets.GetCropId(cropName);
